...
export type TV = {
  ...
  t1: string; t2: string; muted: string;
  ib: string; ibr: string; it: string;
  modal: string; sidebar: string; header: string;
  shadow: string; divider: string; hover: string;
  accent: string; accentText: string; accentHover: string;
};
...
const LIGHT_C: TC = {
  ...
  it:          '#111110',
  accent:      '#1D6EF7',
  accentMuted: 'rgba(29,110,247,0.1)',
};
...
interface ThemeCtx { isDark: boolean; toggle: () => void; v: TV; c: TC; }

const ThemeContext = createContext<ThemeCtx>({
  isDark: false, toggle: () => {}, v: LIGHT, c: LIGHT_C,
});
...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  if (!mounted) {
    return (
      <ThemeContext.Provider value={{ isDark: false, toggle, v: LIGHT, c: LIGHT_C }}>
        {children}
