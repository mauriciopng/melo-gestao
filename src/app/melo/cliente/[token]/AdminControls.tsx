...
export default function AdminControls({ serviceId, initialStages }: Props) {
  ...
  // Verifica se o usuário é admin (tem token válido)
  useEffect(() => {
    const token = tk();
    if (!token) return;
  ...
  async function toggleStageStatus(stage: ServiceStage) {
    const nextStatus = STAGE_CYCLE[stage.status] || 'pendente';
    setUpdatingId(stage.id);
    await fetch('/api/melo/services/stages', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${tk()}` },
      body: JSON.stringify({ serviceId, stageId: stage.id, status: nextStatus }),
    });
    setStages(prev => prev.map(s =>
      s.id === stage.id
        ? { ...s, status: nextStatus, completedAt: nextStatus === 'concluido' ? new Date().toISOString() : s.completedAt }
        : s
    ));
    setUpdatingId(null);
  }
              ...
              return (
                <div key={stage.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: '#F0F0EC',
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontFamily: 'inherit', opacity: isUpdating ? 0.5 : 1,
                      touchAction: 'manipulation',
                    }}
                  >
                    {isUpdating ? '...' :
                      stage.status === 'pendente'     ? '▶ Iniciar' :
                      stage.status === 'em_andamento' ? '✓ Concluir' :
                      '↺ Reabrir'
                    }
                  </button>
            ...
            {!addingComment ? (
              ...
              <form onSubmit={addComment} style={{
                ...
                <textarea
                  ...
                  placeholder="Ex: Fase de demolição iniciada. Prazo estimado 2 dias..."
                  style={{
                    width: '100%', padding: '10px 12px', borderRadius: 8,
                    background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)',
                    color: '#F0F0EC', fontSize: 13, fontFamily: 'inherit',
